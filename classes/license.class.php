...
function getLicenseFromDatabase()
{
	$objectDatabase = $this->objectConnexion;
	$objectDatabase->connect();
	$license_result = mysqli_fetch_object($objectDatabase->query("SELECT * FROM qualification WHERE ID='".$this->reference."'"));
...
function getAircraftLicense($aircraftID)
{
	...
	while ($licenseList = mysqli_fetch_object($list_of_licenses)) {
