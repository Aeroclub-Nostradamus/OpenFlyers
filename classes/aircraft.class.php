...
function getMaxRank()
{
	...
	while ($row = mysqli_fetch_object($aircraftQueryResult)) {
...
function getAircraftFromDatabase()
{
	$objectDatabase = $this->objectConnexion;
	$objectDatabase->connect();
	$aircraftQueryResult = mysqli_fetch_object($objectDatabase->query("SELECT * FROM aircrafts WHERE NUM='".$this->reference."';"));
