...
if ($overtop==0)
{
    ...
    $comments=preg_replace('/[[:cntrl:]]/','',nl2br(htmlentities($comments)));
...
if ($ok_flag)
{
    ...
    $maxBookDateAllowed = clone $almost_now;
...
if ($ok_flag)
{
    ...
    elseif (!$userSession->parameter->isNoOpenTimeLimitation()) {
        ...
        {
            $startOpenTime = clone $start_date;
            $startOpenTime->setClock($interval->begin);
            $endOpenTime = clone $start_date;
        ...
        {
            $startOpenTime = clone $end_date;
            $startOpenTime->setClock($interval->begin);
            $endOpenTime = clone $end_date;
            $endOpenTime->setClock($interval->end);
            echo($end_date->getTS()."/");
            echo($endOpenTime->getTS());
...
if (($ok_flag)and($instructor!=0))
{
    ...
    */
    if ($overtop!=2)
    {
        ...
        for($i=clone $start_date;$i->before($end_date)AND($ok_flag);$i->addSeconds(900))
        {
